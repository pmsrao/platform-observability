...
GROUP BY f.date_sk, c.spark_version, c.major_version, c.minor_version,
         c.cluster_source, c.is_lts, c.is_current, c.runtime_age_months,
         c.months_to_eos, c.is_supported, COALESCE(f.line_of_business, 'Unknown'),
         COALESCE(f.department, 'unknown'), COALESCE(f.cost_center, 'unallocated'),
         COALESCE(f.environment, 'dev');
...
SELECT 
    ...
    CASE 
        WHEN c.worker_node_type_id LIKE '%Standard_DS%' THEN 'General Purpose (DS)'
        WHEN c.worker_node_type_id LIKE '%Standard_E%' THEN 'Memory Optimized (E)'
        WHEN c.worker_node_type_id LIKE '%Standard_F%' THEN 'Compute Optimized (F)'
        WHEN c.worker_node_type_id LIKE '%Standard_L%' THEN 'Storage Optimized (L)'
        WHEN c.worker_node_type_id LIKE '%Standard_N%' THEN 'GPU Enabled (N)'
        WHEN c.worker_node_type_id LIKE '%Standard_M%' THEN 'Memory Optimized (M)'
        ELSE 'Other'
    END as node_family,
    ...
    c.autoscale_enabled,
    CASE 
        WHEN c.max_workers = c.min_workers THEN 'Fixed Size'
    ...
    CASE 
        WHEN c.max_workers = c.min_workers AND c.max_workers > 10 THEN 'Consider Auto-scaling'
        WHEN c.max_workers = c.min_workers AND c.max_workers = 1 THEN 'Single Node - Monitor Performance'
        WHEN c.autoscale_enabled = false AND c.max_workers > c.min_workers THEN 'Enable Auto-scaling'
        ELSE 'Optimal Configuration'
    END as optimization_suggestion
FROM {catalog}.{gold_schema}.gld_fact_usage_priced_day f
JOIN {catalog}.{silver_schema}.slv_clusters c ON f.cluster_identifier = c.cluster_id
GROUP BY f.date_sk, c.driver_node_type_id, c.node_type_id,
         c.min_workers, c.max_workers, c.autoscale_enabled,
...
SELECT 
    ...
    CASE 
        WHEN c.autoscale_enabled = false AND c.max_workers > c.min_workers THEN 'Enable auto-scaling for cost optimization'
        WHEN c.max_workers = c.min_workers AND c.max_workers > 10 THEN 'Consider auto-scaling for variable workloads'
        WHEN c.max_workers = 1 THEN 'Monitor performance - consider multi-node for heavy workloads'
...
GROUP BY f.date_sk, c.workspace_id, c.cluster_id, c.cluster_name,
         c.spark_version, c.major_version, c.minor_version,
...
SELECT 
    ...
    CASE 
        WHEN c.node_type_id LIKE '%Standard_E%' THEN 'Memory Optimized'
        WHEN c.node_type_id LIKE '%Standard_F%' THEN 'Compute Optimized'
        WHEN c.node_type_id LIKE '%Standard_DS%' THEN 'General Purpose'
        ELSE 'Specialized'
    END as node_optimization_type
FROM {catalog}.{gold_schema}.gld_fact_usage_priced_day f
JOIN {catalog}.{silver_schema}.slv_clusters c ON f.cluster_identifier = c.cluster_id
WHERE c.spark_version IS NOT NULL
GROUP BY f.date_sk, c.spark_version, c.major_version, c.minor_version,
...
SELECT 
    ...
    CASE 
        WHEN c.max_workers = c.min_workers AND c.max_workers > 10 THEN 'Consider auto-scaling for variable workloads'
        WHEN c.max_workers = c.min_workers AND c.max_workers = 1 THEN 'Monitor performance - consider multi-node for heavy workloads'
        WHEN c.autoscale_enabled = false AND c.max_workers > c.min_workers THEN 'Enable auto-scaling for cost optimization'
        ELSE 'Current configuration is optimal'
    END as sizing_recommendation,
    -- Cost optimization potential
    CASE 
        WHEN c.autoscale_enabled = false AND c.max_workers > c.min_workers THEN 'Medium - Auto-scaling can reduce costs by 15-30%'
        WHEN c.max_workers = c.min_workers AND c.max_workers > 10 THEN 'Medium - Consider auto-scaling for variable workloads'
        ELSE 'Low - Current configuration is optimal'
    END as optimization_potential
FROM {catalog}.{gold_schema}.gld_fact_usage_priced_day f
JOIN {catalog}.{silver_schema}.slv_clusters c ON f.cluster_identifier = c.cluster_id
GROUP BY f.date_sk, c.workspace_id, c.cluster_id, c.cluster_name, c.cluster_type,
         c.node_type_id, c.min_workers, c.max_workers, c.autoscale_enabled;
